...
| 8  | Ingest scanner — scans `staged/**` + `pages/**` | `src/builder/ingest.js`                       | 1.4   |
...
| 12 | Hash diff + manifest (incremental build)        | `src/cms/index.js`, `src/builder/pipeline.js` | 1.5   |
| 13 | Route injector                                  | `src/builder/routes-inject.js`                | 1.7   |

**Lưu ý ngoài bảng:**

- **`archived/` redirect stub** là sub-requirement của Story 1.4 (không phải module riêng): khi `ingest.js`/`pipeline.js` gặp bài trong `content/posts/archived/`, build phải emit redirect stub HTML để URL không 404.
- **Story 1.10 — index.json pagination** (`src/cms/index.js`, `src/builder/pipeline.js`) là **trigger-based, post-MVP**: chỉ implement khi tổng số bài > 5,000. Không nằm trong thứ tự module MVP ở trên, nhưng cần nhớ khi Module #12 (index.js) đã ổn định.
...
│   ├── en.md                     ← body hợp lệ, vấn đề chỉ ở meta.yaml
│   └── meta.yaml
├── staged/2026/06/01/00/04/      ← title có dấu hai chấm "Final Whistle: How a Last-Minute Penalty Decided the Derby"
│   └── meta.yaml
...
```text
✅ npm run test:cms → tất cả tests pass
