...
- `cms/config.yaml` exists with ALL required keys:
  ...
  - `analytics.ga4_measurement_id` — GA4 ID (inject in head)
  - `quality_gate.min_word_count: 300` and `quality_gate.warn_word_count: 600`
...
- **Quality gate (automation without human):**
  - `word_count < 300` → ERROR: skip + `errors.log { code: "THIN_CONTENT", wordCount, dir }` → AI reads log, auto-retries
  - `300 ≤ word_count < 600` → WARNING: publish + log `{ code: "SHORT_CONTENT", wordCount }`
...
- 10 articles: 1 thin (<300w), 1 future-dated, 1 missing-title, 1 in draft/ → 6 built, 2 errors, 2 skipped
