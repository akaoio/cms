...
test("config", async (t) => {
    await t.test("loadConfig — loads cms/config.json and exposes required keys", async () => {
        const config = await loadConfig(fixture("valid.json"))
    ...
    await t.test("loadConfig — does not throw when optional ga4_measurement_id is absent", async () => {
        const config = await loadConfig(fixture("no-analytics.json"))
    ...
    await t.test("loadConfig — throws when site.default_og_image is missing", async () => {
        await assert.rejects(() => loadConfig(fixture("missing-og-image.json")))
    })

    await t.test("loadConfig — throws when quality_gate.min_word_count is missing", async () => {
        await assert.rejects(() => loadConfig(fixture("missing-min-word-count.json")))
    })
})
