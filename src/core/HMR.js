...
if (DEV) {
    class HMRRuntime {
        constructor() {
            ...
            if (!window.hmr?.origdefine) this.setup()
        }
        ...
        register(url, exports, dependencies = []) {
            ...
            for (const dep of dependencies) {
                ...
                if (depModule) depModule.dependents.add(url)
            }
        }
        ...
        async apply({ path, type, timestamp }) {
            ...
            if (type === "css" || path.endsWith(".css.js")) await this.swapcss(url, timestamp)
            else if (type === "template" || path.includes("/template.js")) await this.swaptpl(url, timestamp)
            else if (type === "js" || path.endsWith(".js")) await this.swapmod(url, timestamp)
        }
        ...
        async swapmod(url, timestamp) {
            try {
                ...
                if (data) {
                    ...
                    for (const dep of data.dependents) await this.swapmod(dep, timestamp)
                }
        ...
        async swapcomp(tag, module, url) {
            ...
            const Cls = module[Object.keys(module).find((k) => module[k]?.prototype instanceof HTMLElement)] || module.default
            ...
            const baseMethods = new Set(["constructor", "connectedCallback", "disconnectedCallback", "onConnect", "onDisconnect", "subscribe", "listen", "watch", "query", "queryAll", "renderTemplate"])
            ...
            for (const el of live) {
                ...
                try {
                    ...
                    for (const method of methods) {
                        ...
                        if (typeof Cls.prototype[method] === "function") el[method] = Cls.prototype[method].bind(el)
                    }
                    ...
                    // Restore states
                    if (el.states && Object.keys(states).length > 0) el.states.set(states)

                    // Preserve subscriptions (Component manages these)
                    if (subs.length > 0) el.subscriptions = subs
                } catch (error) {
        ...
        findelem(url) {
            ...
            for (const [tag, data] of this.elements.entries()) if (data.module?.includes(url) || url.includes(data.module)) results.push({ tagName: tag, ...data })
         ...
         */
        findtag(url) {
            for (const [tag, data] of this.elements.entries()) if (data.module === url || url.includes(data.module?.replace(/\\/g, "/"))) return tag
    ...
    if (window.hmr && window.hmr.elements) {
        ...
        for (const key of Object.getOwnPropertyNames(HMRRuntime.prototype)) if (key !== "constructor") hmr[key] = runtime[key].bind(hmr)
    ...
    if (typeof window !== "undefined") window.hmr = hmr
